...
public class CompraDAO {
    ...
    private static final String sqlconsultaclientes = "SELECT * FROM \"compras\" order by produto";
    private static final String sqlinserir = "INSERT INTO compras (id, produto, cliente, preco) VALUES (?, ?, ?, ?)";
    private static final String sqlalterar = "UPDATE compras SET produto = ?, cliente = ?, preco = ? WHERE id = ?";
    private static final String sqlaexcluir = "DELETE FROM compras WHERE id = ?";
    ...
    public boolean Alterar(Compra cli) {
       try {
            int tipo = ResultSet.TYPE_SCROLL_SENSITIVE;
            int concorrencia = ResultSet.CONCUR_UPDATABLE;
            pstdados = connection.prepareStatement(sqlalterar, tipo, concorrencia);
            pstdados.setString(1, cli.getProduto());
            pstdados.setString(2, cli.getCliente());
    ...
    public boolean Excluir(Compra cli) {
        try {
            int tipo = ResultSet.TYPE_SCROLL_SENSITIVE;
            int concorrencia = ResultSet.CONCUR_UPDATABLE;
    ...
    public Compra getCompra() {
        ...
        if (rsdados != null) {
            try {
                ...
                String prod = rsdados.getString("produto");
                double prec = rsdados.getDouble("preco");
                String clie = rsdados.getString("cliente");
                cli = new Compra(id, prod, prec, clie);
            } catch (SQLException erro) {
